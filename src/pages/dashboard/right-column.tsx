...
export const RightColumn: FC<Props> = observer(
  ({ rightColumnOpen, toggleRightColumn, user }) => {
      ...
      >
        <div className="flex flex-row items-center">
          ...
          <p className="leading-tight ml-2 flex flex-col flex-1">
            <span
              className={`${user?._id && "text-lg opacity-60"} font-medium`}
            >
          ...
          <Button
            onClick={() => rightColumnOpen && toggleRightColumn(false)}
            shape="circle"
            icon={<i className="icon-CancelMajor" />}
          />
        </div>
...
const UserMutation: FC<{ user?: User; onSuccess: (open: boolean) => void }> = ({
  ...
  const { mutate: createDashUserMutation, loading: creating } = useMutation(
    userApi.addDashUser,
    {
      onSuccess: ({ data }) => {
        onSuccess(true);
  ...
  const { mutate: updateUserMutation, loading: updating } = useMutation(
    userApi.updateAdminUser,
    {
    ...
    <>
      <Controller
        ...
        render={({ field: { onChange, value }, fieldState: { error } }) => (
          <>
            <p
              className={`px-1 mb-1 font-medium mt-6 ${
                false && "text-red-600"
              }`}
            >
              Your name*
      ...
      <Controller
        ...
        render={({ field: { onChange, value }, fieldState: { error } }) => (
          <>
            <p className={`px-1 mb-1 mt-4 ${error && "text-red-600"}`}>
              Address line*
            </p>
            <Input.TextArea
              autoSize={{ minRows: 1 }}
      ...
      <div className="flex flex-row">
        <div className="w-3/5 pr-2">
          <Controller
            ...
            render={({ field: { onChange, value }, fieldState: { error } }) => (
              <>
                <p className={`px-1 mb-1 mt-2 ${error && "text-red-600"}`}>
                  State*
                </p>
                <Input
                  className="w-full"
